...
- `render_with_compute` - Renders an image using compute shaders. Note that the performance of this example will be poor on Firefox webgpu due to a limitation in the browser, this is explained at the top of the file and can easily be prevented by not waiting for the GPU to complete after every submit.
