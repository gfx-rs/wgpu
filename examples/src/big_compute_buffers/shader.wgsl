...
@group(0) @binding(0)
var<storage, read_write> contigious_array: binding_array<ContigiousArray,NUM_BUFFERS>;
...
fn main(@builtin(global_invocation_id) global_id: vec3<u32>) {
    ...
    for (var i = 0u; i < OFFSET; i++) {
        ...
        if (index < TOTAL_SIZE) {
            ...
            contigious_array[buffer_index].inner[inner_index] = add_one(contigious_array[buffer_index].inner[inner_index]);
        }
    }
}
