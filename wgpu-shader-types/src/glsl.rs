...
impl GlslVersion {
    ...
    pub const fn is_es(&self) -> bool {
        match *self {
            GlslVersion::Desktop(_) => false,
            GlslVersion::Embedded { .. } => true,
        }
    }
    ...
    pub const fn is_webgl(&self) -> bool {
        match *self {
            GlslVersion::Desktop(_) => false,
            GlslVersion::Embedded { is_webgl, .. } => is_webgl,
        }
    }
    ...
    pub fn is_supported(&self) -> bool {
        match *self {
            GlslVersion::Desktop(v) => SUPPORTED_CORE_VERSIONS.contains(&v),
            GlslVersion::Embedded { version: v, .. } => SUPPORTED_ES_VERSIONS.contains(&v),
        }
    }

    pub fn supports_io_locations(&self) -> bool {
        *self >= GlslVersion::Desktop(330) || *self >= GlslVersion::new_gles(300)
    }
    ...
    pub fn supports_explicit_locations(&self) -> bool {
        *self >= GlslVersion::Desktop(420) || *self >= GlslVersion::new_gles(310)
    }

    pub fn supports_early_depth_test(&self) -> bool {
        *self >= GlslVersion::Desktop(130) || *self >= GlslVersion::new_gles(310)
    }

    pub fn supports_std140_layout(&self) -> bool {
        *self >= GlslVersion::Desktop(140) || *self >= GlslVersion::new_gles(300)
    }

    pub fn supports_std430_layout(&self) -> bool {
        *self >= GlslVersion::Desktop(430) || *self >= GlslVersion::new_gles(310)
    }

    pub fn supports_fma_function(&self) -> bool {
        *self >= GlslVersion::Desktop(400) || *self >= GlslVersion::new_gles(320)
    }

    pub fn supports_integer_functions(&self) -> bool {
        *self >= GlslVersion::Desktop(400) || *self >= GlslVersion::new_gles(310)
    }
    ...
    pub fn supports_pack_unpack_unorm_2x16(&self) -> bool {
        *self >= GlslVersion::Desktop(400) || *self >= GlslVersion::new_gles(300)
    }
...
impl PartialOrd for GlslVersion {
    fn partial_cmp(&self, other: &Self) -> Option<Ordering> {
        match (*self, *other) {
            (GlslVersion::Desktop(x), GlslVersion::Desktop(y)) => Some(x.cmp(&y)),
            (
                GlslVersion::Embedded { version: x, .. },
                GlslVersion::Embedded { version: y, .. },
            ) => Some(x.cmp(&y)),
            _ => None,
        }
    }
...
impl fmt::Display for GlslVersion {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match *self {
            GlslVersion::Desktop(v) => write!(f, "{v} core"),
            GlslVersion::Embedded { version: v, .. } => write!(f, "{v} es"),
        }
    }
}
...
/// Separate type from naga so that naga can easily add impl's
#[derive(Debug, Copy, Clone, PartialEq, Eq, Hash)]
#[cfg_attr(feature = "serialize", derive(serde::Serialize))]
#[cfg_attr(feature = "deserialize", derive(serde::Deserialize))]
