...
- Recover from `VK_ERROR_FRAGMENTED_POOL` when allocating descriptor sets by retiring the offending pool and retrying on the next available (or a freshly created) pool instead of panicking; `VK_ERROR_OUT_OF_POOL_MEMORY` is now surfaced as an unexpected backend error instead of panicking. By @beicause in [#10264](https://github.com/gfx-rs/wgpu/pull/10264).
