...
- Added experimental `GPUResourceTable`-style resource tables (bindless): `Device::create_resource_table` creates a `ResourceTable`, whose slots are populated with `update`/`insert_binding`/`remove_binding` (a slot being consumed by an in-flight submission returns `ResourceTableError::SlotInUse` until that submission completes). Bind a table to a compute pass with `ComputePass::set_resource_table`, or to a render pass via `RenderPassDescriptor::resource_table`; pipelines that read from a table need a layout created with `PipelineLayoutDescriptor::uses_resource_table` set. In WGSL, `enable resource_table;` unlocks `getResource<T>(index)`, which looks up a texture directly by `u32` index without a bind group. Gated behind the native-only, experimental `Features::EXPERIMENTAL_SAMPLING_RESOURCE_TABLE`; pipelines using `getResource` additionally require `Features::EXPERIMENTAL_RESOURCE_TABLE_UNCHECKED`, since this milestone has no checked/bounds-safe lowering yet. Only sampled and depth textures can be table members for now, and a submission is rejected if a table member is used, in the same command buffer, in a way that would force it into an image layout incompatible with table sampling (plain read-only sampling, and read-only depth-attachment use of a depth member, are fine). Implemented on Vulkan only; other backends are not yet implemented. By @cwfitzgerald.
