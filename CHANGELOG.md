...
- Fix WGSL `select` with a vector condition generating GLSL `mix` for integer, unsigned, and boolean components, which those types' `mix` overloads do not support below GLSL 4.50 / GLSL ES 3.10. Affected WebGL2 and older desktop GL targets. By @arcusbuilds in [#PRNUM](https://github.com/gfx-rs/wgpu/pull/PRNUM).
