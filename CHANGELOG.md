...
- Add subgroup size control for SPIR-V passthrough shaders via a new `subgroup_size: SubgroupSize` field on `PassthroughShaderEntryPoint`, gated behind the new `Features::SUBGROUP_SIZE_CONTROL`. The `SubgroupSize` enum offers `Varying` (default), `Full` (compute/task/mesh stages only), and `Fixed(u32)`. Validation runs at shader-module creation: for `Fixed(n)` it follows the WebGPU [`subgroup-size-control` proposal](https://github.com/gpuweb/gpuweb/blob/main/proposals/subgroup-size-control.md) — power of two within `[subgroup_min_size, subgroup_max_size]`, and the entry point's `workgroup_size.x` must be a multiple of `n`. When `subgroup_size` is non-`Varying`, `PassthroughShaderEntryPoint::workgroup_size` must be populated (with a non-zero `x` component matching the shader's declared local size) so module-creation validation has the value to check against. Honored on Vulkan via `VK_EXT_subgroup_size_control` (promoted in 1.3); non-`Varying` values are rejected when the descriptor carries any non-SPIR-V source, since neither HLSL (which uses `[WaveSize(n)]` in source) nor MetalLib/MSL has an API to control the subgroup width, and silently dropping the request on backends that consume those sources would be surprising. `Full` on a vertex or fragment stage is rejected at render-pipeline creation; for `Fixed(n)`, the set of stages that can carry a required subgroup size is implementation-dependent (Vulkan's `requiredSubgroupSizeStages`), so wgpu forwards the request and lets the driver reject unsupported stages. For WGSL/Naga shaders, prefer the proposal's `@subgroup_size` attribute once it lands. By @ruihe774 in [#9523](https://github.com/gfx-rs/wgpu/pull/9523).
