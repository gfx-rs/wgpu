...
- Added `Features::TEXTURE_COMPRESSION_UNALIGNED` (WebGPU's `texture-compression-unaligned`), which allows creating block-compressed textures whose size is not a multiple of the texel block dimensions. Supported on all backends, except on WebGL it requires BC textures to be unavailable (their WebGL extensions require block-aligned sizes), and on DX12 it requires `UnalignedBlockTexturesSupported` from `D3D12_FEATURE_DATA_D3D12_OPTIONS8`. By @castano in [#10247](https://github.com/gfx-rs/wgpu/pull/10247).
