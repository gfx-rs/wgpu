...
impl crate::NagaShader {
    pub fn compile_spv(
        &self,
        desc: SpvShaderDesc,
    ) -> Result<Vec<u32>, super::ShaderCompilationError> {
        #[cfg(feature = "naga-dep")]
        {
            ...
            let (module, info): (Cow<naga::Module>, Cow<naga::valid::ModuleInfo>) =
                if desc.entry_point.is_some() {
                    ...
                    .map_err(|e| super::ShaderCompilationError::PipelineConstants(format!("{e}")))?
                } else {
            ...
            .map_err(|e| super::ShaderCompilationError::Linkage(format!("{e}")))
        }
        #[cfg(not(feature = "naga-dep"))]
        {
            unreachable!()
