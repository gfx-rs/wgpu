...
fn scissor_test_impl(ctx: &TestingContext, scissor_rect: Rect, expected_data: [u8; 16]) {
    ...
    let shader = ctx.device.create_shader_module(wgpu::include_wgsl!("solid_white.wgsl"));
...

    {
        let mut encoder = ctx.device.create_command_encoder(&wgpu::CommandEncoderDescriptor { label: None });
        {
            ...
            render_pass.set_scissor_rect(scissor_rect.x, scissor_rect.y, scissor_rect.width, scissor_rect.height);
            render_pass.set_index_buffer(index_buf.slice(..), wgpu::IndexFormat::Uint16);
            render_pass.set_vertex_buffer(0, vertex_buf.slice(..));
            render_pass.draw_indexed(0..(index_buf.size() as usize / size_of::<i16>()) as u32, 0, 0..1);
        }
        ctx.queue.submit(Some(encoder.finish()));
    }
    let data = wgpu_test::image::capture_rgba_u8_texture(ctx, &texture, wgpu::Extent3d {
        width: 2,
        height: 2,
        depth_or_array_layers: 1,
    });
