...
#[gpu_test]
static ZERO_INIT_WORKGROUP_MEMORY_THREADS: GpuTestConfiguration = GpuTestConfiguration::new()
    .parameters(
    ...
    .run_async(|ctx| async move {
        ...
        let sm = ctx
            .device
            .create_shader_module(include_wgsl!("zero_init_workgroup_mem_threads.wgsl"));
        ...
        let num_dispatches_failed = typed.iter().filter(|&&res| res != 0).count();
        let ratio = (num_dispatches_failed as f32 / OUTPUT_ARRAY_SIZE as f32) * 100.;

        assert!(
            num_dispatches_failed == 0,
