...
/// We have an OpenGL bug that, when running without explicit in-shader locations,
/// we will not properly bind uniform buffers to both the vertex and fragment
/// shaders. Additionally, push constants with different ranges and names
...
fn multi_stage_data_binding_test(ctx: TestingContext) {
    ...
    let input_as_unorm = [25_u8, 50, 75, 100];
