...
fn main_1() {
    let _e10: mat4x4<f32> = global_1.Model;
    let _e11: vec3<f32> = Vertex_Normal_1;
    v_Normal = (_e10 * vec4<f32>(_e11, 1.0)).xyz;
    let _e16: mat4x4<f32> = global_1.Model;
    let _e26: vec3<f32> = Vertex_Normal_1;
    v_Normal = (mat3x3<f32>(_e16[0].xyz, _e16[1].xyz, _e16[2].xyz) * _e26);
    let _e28: mat4x4<f32> = global_1.Model;
    let _e29: vec3<f32> = Vertex_Position_1;
    v_Position = (_e28 * vec4<f32>(_e29, 1.0)).xyz;
    let _e34: vec2<f32> = Vertex_Uv_1;
    v_Uv = _e34;
    let _e36: mat4x4<f32> = global.ViewProj;
    let _e37: vec3<f32> = v_Position;
