        ...
        let terminator = loop {
            ...
            match inst.op {
                ...
                Op::AtomicFlagTestAndSet => {
                    // Atomic flag test and set is essentially an atomic exchange of a
                    // literal "true"
                    inst.expect(6)?;
                    let start = self.data_offset;
                    let result_type_id = self.next()?;
                    ...
                    let value = ctx
                        .expressions
                        .append(crate::Expression::Literal(crate::Literal::Bool(true)), span);
                    let stmt = crate::Statement::Atomic {
                        pointer,
                        fun: crate::AtomicFunction::Exchange { compare: None },
                        value,
                        result,
                    };
                    block.push(stmt, span);
                ...
                Op::AtomicFlagClear => {
                    ...
                    block.extend(emitter.finish(ctx.expressions));
                    // Create a literal "false" to use as our store value
