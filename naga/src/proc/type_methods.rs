...
/// Return `true` if the binary operator `op` accepts operands of types `left`
/// and `right`.
///
/// Both types must be concrete: abstract scalars are never accepted, since
/// abstract values must be concretized before they reach a runtime operation.
///
/// This is the single source of truth for Naga IR's binary operator typing
/// rules: it is used both by the IR validator and by the WGSL front end, so
/// that the front end can report operand type mismatches in WGSL terms.
pub fn binary_op_accepts_operands(
    op: crate::BinaryOperator,
    left: &crate::TypeInner,
    right: &crate::TypeInner,
) -> bool {
    ...
    match op {
        Bo::Add | Bo::Subtract => match *left_inner {
            ...
            Ti::Matrix { .. } | Ti::CooperativeMatrix { .. } => left_inner == right_inner,
            _ => false,
        },
        ...
        Bo::Multiply => {
            ...
            let types_match = match (left_inner, right_inner) {
                ...
                // Matrix * matrix.
                (&Ti::Matrix { columns, .. }, &Ti::Matrix { rows, .. }) => columns == rows,
                // Scalar * coop matrix.
                (&Ti::Scalar(s1), &Ti::CooperativeMatrix { scalar: s2, .. })
                | (&Ti::CooperativeMatrix { scalar: s1, .. }, &Ti::Scalar(s2)) => s1 == s2,
                _ => false,
            };
        ...
        Bo::Less | Bo::LessEqual | Bo::Greater | Bo::GreaterEqual => match *left_inner {
            Ti::Scalar(scalar) | Ti::Vector { scalar, .. } => match scalar.kind {
                Sk::Uint | Sk::Sint | Sk::Float => left_inner == right_inner,
                Sk::Bool | Sk::AbstractInt | Sk::AbstractFloat => false,
            },
