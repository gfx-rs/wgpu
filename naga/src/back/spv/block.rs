...
use super::{
    helpers::map_storage_class, index::BoundsCheckResult, selection::Selection, AccessKind, Block,
...
impl BlockContext<'_> {
    ...
    pub(super) fn cache_expression_value(
        ...
        let id = match self.ir_function.expressions[expr_handle] {
            ...
            crate::Expression::CooperativeLoad { ref data, .. } => {
                ...
                let memory_operands = self.coop_memory_operands(data.pointer, AccessKind::Load);
    ...
    fn write_checked_load(
            ...
            self.maybe_write_load_uniform_matcx2_struct_member(pointer, block)?
        {
            ...
            let pointer_ty = self.fun_info[pointer].ty.inner_with(&self.ir_module.types);
            let is_atomic = pointer_ty.is_atomic_pointer(&self.ir_module.types);
            let pointer_space = pointer_ty.pointer_space();
            let memory_operands = match pointer_space {
                Some(space) if !is_atomic => {
                    self.access_memory_operands(pointer, space, AccessKind::Load)
                }
                _ => None,
            };
        ...
        for (statement, span) in naga_block.span_iter() {
            ...
            match *statement {
                ...
                Statement::Store { pointer, value } => {
                    ...
                    let memory_operands = match pointer_space {
                        Some(space) if !is_atomic => {
                            self.access_memory_operands(pointer, space, AccessKind::Store)
                        }
                        _ => None,
                    };
                ...
                Statement::CooperativeStore { target, ref data } => {
                    ...
                    let stride_id = self.cached[data.stride];
                    let memory_operands =
                        self.coop_memory_operands(data.pointer, AccessKind::Store);
                    match self.write_access_chain(
                        data.pointer,
                        &mut block,
                        AccessTypeAdjustment::None,
