...
fn convert_spv_all() {
    ...
    let atomic_targets = Targets::IR | Targets::METAL | Targets::WGSL;
    convert_spv("atomic_i_increment", false, atomic_targets);
    convert_spv("atomic_load_and_store", false, atomic_targets);
    convert_spv("atomic_exchange", false, atomic_targets);
    convert_spv("atomic_compare_exchange", false, atomic_targets);
    convert_spv("atomic_i_decrement", false, atomic_targets);
    convert_spv("atomic_i_add_sub", false, atomic_targets);
    convert_spv(
        "fetch_depth",
        false,
