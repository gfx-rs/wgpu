...
#[test]
fn loop_local_var_without_initializer_is_zeroed_each_iteration() {
    let module = naga::front::wgsl::parse_str(
        r#"
        fn main() {
            loop {
    ...
    let function = module
        ...
        .1;
    let loop_body = match &function.body[0] {
        naga::Statement::Loop { body, .. } => body,
    ...
    let local = function
        ...
        .iter()
        .next()
        .expect("function should contain a local variable")
        .0;
    let store = match &loop_body[0] {
        naga::Statement::Store { pointer, value } => (*pointer, *value),
        other => panic!("expected store statement, found {other:?}"),
    };

    assert_eq!(
        function.local_variables[local].init, None,
        "loop-local implicit initialization should not be hoisted"
    );
    assert!(matches!(
        function.expressions[store.0],
        naga::Expression::LocalVariable(handle) if handle == local
    ));
    assert!(matches!(
        function.expressions[store.1],
        naga::Expression::ZeroValue(ty) if ty == function.local_variables[local].ty
    ));
}
