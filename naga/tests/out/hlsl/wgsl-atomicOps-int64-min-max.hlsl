...
{
    uint64_t _e3 = input;
    storage_atomic_scalar.InterlockedMax(0, _e3);
    uint64_t _e7 = input;
    storage_atomic_arr.InterlockedMax(8, (1uL + _e7));
    storage_struct.InterlockedMax(0, 1uL);
    storage_struct.InterlockedMax(8+8, uint64_t(id.x));
    GroupMemoryBarrierWithGroupSync();
    uint64_t _e20 = input;
    storage_atomic_scalar.InterlockedMin(0, _e20);
    uint64_t _e24 = input;
    storage_atomic_arr.InterlockedMin(8, (1uL + _e24));
    storage_struct.InterlockedMin(0, 1uL);
    storage_struct.InterlockedMin(8+8, uint64_t(id.x));
    return;
}
