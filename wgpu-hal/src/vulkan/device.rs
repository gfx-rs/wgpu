...
impl super::DeviceShared {
    ...
    fn make_memory_ranges<'a, I: 'a + Iterator<Item = crate::MemoryRange>>(
        ...
        let block = buffer.allocation.as_ref()?;
...
impl crate::Device for super::Device {
    ...
    unsafe fn create_buffer(
        ...
        let alloc_desc = gpu_allocator::vulkan::AllocationCreateDesc {
            name: desc.label.unwrap_or("Unlabeled buffer"),
            // TODO: Previously used `req.memory_type_bits & self.valid_ash_memory_types` to set `memory_type_bits`,
            // but according to the spec "Bit i is set if and only if the memory type i in the
            // VkPhysicalDeviceMemoryProperties structure for the physical device is supported for the resource."
            // https://registry.khronos.org/vulkan/specs/latest/man/html/VkMemoryRequirements.html
            requirements: req,
            location,
            // It's a buffer, use linear
            linear: true,
            allocation_scheme: gpu_allocator::vulkan::AllocationScheme::GpuAllocatorManaged,
        };

        let allocation = self.mem_allocator.lock().allocate(&alloc_desc)?;
    ...
    unsafe fn destroy_buffer(&self, buffer: super::Buffer) {
        ...
        if let Some(allocation) = buffer.allocation {
            ...
            match self.mem_allocator.lock().free(allocation) {
                Ok(()) => (),
                Err(e) => panic!("Failed to destroy vulkan buffer, {e}"),
            }
        }
    ...
    unsafe fn unmap_buffer(&self, buffer: &super::Buffer) {
        // TODO: map and unmap_buffer are probably wrong
        // if let Some(ref block) = buffer.allocation {
    ...
    unsafe fn create_texture(
        ...
        let alloc_desc = gpu_allocator::vulkan::AllocationCreateDesc {
            name: desc.label.unwrap_or("Unlabeled texture"),
    ...
    unsafe fn destroy_texture(&self, texture: super::Texture) {
        ...
        if let Some(block) = texture.block {
            ...
            match self.mem_allocator.lock().free(block) {
                Ok(()) => (),
                Err(e) => panic!("Failed to destroy vulkan texture: {}", e),
            };
        }
    ...
    unsafe fn create_acceleration_structure(
        ...
        unsafe {
            ...
            let allocation_desc = gpu_allocator::vulkan::AllocationCreateDesc {
                name: desc.label.unwrap_or("rt_acceleration_structure"),
                requirements: req,
                location: gpu_allocator::MemoryLocation::GpuOnly,
                // idk?
                linear: true,
            ...
            let allocation = self.mem_allocator.lock().allocate(&allocation_desc)?;
