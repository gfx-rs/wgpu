...
const REC709: [[f32; 2]; 3] = [[0.640, 0.330], [0.300, 0.600], [0.150, 0.060]];
const DISPLAY_P3: [[f32; 2]; 3] = [[0.680, 0.320], [0.265, 0.690], [0.150, 0.060]];
const REC2020: [[f32; 2]; 3] = [[0.708, 0.292], [0.170, 0.797], [0.131, 0.046]];
...
fn classify_gamut(red: [f32; 2], green: [f32; 2], blue: [f32; 2]) -> Option<wgt::DisplayGamut> {
    ...
    let dist = |reference: &[[f32; 2]; 3]| -> f32 {
        measured
            .iter()
            .zip(reference)
            .map(|(m, r)| {
...
/// Maps an [`IDXGIOutput6::GetDesc1`] result into the backend-agnostic
/// [`wgt::DisplayHdrInfo`].
///
/// Pure and infallible: it touches no OS state (the caller already did the
/// fallible `GetDesc1`), so it cannot fail or panic. All numeric values are
/// advisory (EDID-sourced); see [`wgt::DisplayLuminance`].
///
/// `sdr_white_nits` is *not* part of `GetDesc1` (it lives in the DisplayConfig
/// API); the caller obtains it separately via [`sdr_white_nits_for_monitor`] and
/// passes it in, keeping this mapping pure.
///
...
pub fn display_hdr_info_from_desc1(
    ...
    let mut luminance = wgt::DisplayLuminance::default();
    luminance.max_nits = Some(desc1.MaxLuminance);
    luminance.max_full_frame_nits = Some(desc1.MaxFullFrameLuminance);
    luminance.min_nits = Some(desc1.MinLuminance);
    // The absolute↔relative bridge value, sourced from DisplayConfig (not
    // `GetDesc1`); `None` if that query was unavailable.
    luminance.sdr_white_nits = sdr_white_nits;

    let mut chromaticity = wgt::DisplayChromaticity::default();
    chromaticity.red = Some(desc1.RedPrimary);
    chromaticity.green = Some(desc1.GreenPrimary);
    chromaticity.blue = Some(desc1.BluePrimary);
    chromaticity.white = Some(desc1.WhitePoint);

    let mut coarse = wgt::DisplayCoarseRange::default();
    coarse.high_dynamic_range = Some(hdr_active);
    coarse.gamut = classify_gamut(desc1.RedPrimary, desc1.GreenPrimary, desc1.BluePrimary);

    let mut info = wgt::DisplayHdrInfo::default();
    info.hdr_active = Some(hdr_active);
    info.luminance = Some(luminance);
    info.chromaticity = Some(chromaticity);
    info.coarse = Some(coarse);
    // `BitsPerColor` is advisory and occasionally absurd; only surface a value
    // that fits the `u8` field, otherwise leave it unreported.
    info.bits_per_color = u8::try_from(desc1.BitsPerColor).ok();
    // `headroom` stays `None`: Windows reports the absolute frame, not Apple's
    // relative EDR multipliers.
    info
}
...
/// adapter.
///
/// Returns `None` if the output cannot be identified or queried — a headless /
...
///
/// Returns `None` on any failure (no match, query error, or a nonsensical `0`).
/// Never panics. Pass [`Dxgi::DXGI_OUTPUT_DESC1::Monitor`] (the same monitor the
/// descriptor came from) so the value matches the rest of the snapshot.
