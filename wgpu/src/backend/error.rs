    ...
    fn fmt_pretty(&self, context: &super::Context) -> String {
        ...
        match self {
            Self::InvalidBindGroup(id) => {
                let name = wgc::gfx_select2!(&Arc id => super::direct::Global::bind_group_label(id));
                ret.push_str(&format_label_line("bind group", &name));
            }
            Self::InvalidPipeline(id) => {
                let name = wgc::gfx_select2!(&Arc id => super::direct::Global::render_pipeline_label(id));
...
impl PrettyError for wgc::binding_model::CreateBindGroupError {
    fn fmt_pretty(&self, context: &super::Context) -> String {
        ...
        match *self {
            ...
            Self::InvalidSampler(/*ref id*/bind_group) => {
                //let name = wgc::gfx_select2!(&Arc id => super::direct::Global::sampler_label(id));
                ret.push_str(&format_note_line(&format!("sampler (bind group = `{}`)", bind_group)));
                // ret.push_str(&format_label_line("sampler at bind group", /*&name*/bind_group));
            }
            _ => {}
        };
        ret
...
impl PrettyError for wgc::binding_model::CreatePipelineLayoutError {
    fn fmt_pretty(&self, _context: &super::Context) -> String {
        // let global = context.global();
        let mut ret = format_error_line(self);
        if let Self::InvalidBindGroupLayout(/*id*/bind_group_layout) = self {
            // let name = wgc::gfx_select2!(&Arc id => super::direct::Global::bind_group_layout_label(id));
            ret.push_str(&format_note_line(&format!("bind group layout (index = `{}`)", bind_group_layout)));
...
impl PrettyError for wgc::command::ComputePassErrorInner {
    fn fmt_pretty(&self, context: &super::Context) -> String {
        ...
        match self {
            ...
            Self::InvalidPipeline(id) => {
                let name = wgc::gfx_select2!(&Arc id => super::direct::Global::compute_pipeline_label(id));
