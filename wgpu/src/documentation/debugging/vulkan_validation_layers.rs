...
layers here: <https://gpuopen.com/learn/using-the-vulkan-validation-layers/>.
...
Download the Vulkan SDK from <https://vulkan.lunarg.com/sdk/home> and follow
the installer instructions for your platform.
...
*not* passed to `cargo`), because [`InstanceFlags::VALIDATION`] is set by
default in development builds. You can also set that flag explicitly, or set
the environment variable `VK_INSTANCE_LAYERS=VK_LAYER_KHRONOS_validation`.
