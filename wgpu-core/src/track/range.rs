...
impl<I: Copy + PartialOrd, T: Copy + PartialEq> RangedStates<I, T> {
    ...
    pub fn merge<'a>(&'a self, other: &'a mut Self, base: I) -> Merge<'a, I, T> {
        Merge {
            base,
            max: self.ranges.last().unwrap().0.end,
            index_a: 0,
            slice_a: &self.ranges,
            index_b: 0,
            slice_b: &mut other.ranges,
        }
    }
}

pub(crate) struct MergeItem<'a, T>(pub T, pub &'a mut T);
...
pub(crate) struct Merge<'a, I, T> {
    ...
    slice_a: &'a [(Range<I>, T)],
    index_b: usize,
    slice_b: &'a mut [(Range<I>, T)],
}

impl<'a, I: Ord + Copy, T: Copy + Debug> Iterator for Merge<'a, I, T> {
    type Item = (Range<I>, MergeItem<'a, T>);
    fn next(&mut self) -> Option<Self::Item> {
        if self.base >= self.max {
            return None;
        }

        let a_end = self.slice_a[self.index_a].0.end;
        let b_end = self.slice_b[self.index_b].0.end;

        let a_item = self.slice_a[self.index_a].1;
        let b_item = unsafe { &mut (*self.slice_b.as_mut_ptr().add(self.index_b)).1 };

        match a_end.cmp(&b_end) {
            Ordering::Less => {
                self.index_a += 1;

                let range = self.base..a_end;
                self.base = a_end;

                Some((range, MergeItem(a_item, b_item)))
            }
            Ordering::Equal => {
                self.index_a += 1;
                self.index_b += 1;
