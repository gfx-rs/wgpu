...
impl Global {
    ...
    pub fn device_create_command_encoder(
        ...
        let error = 'error: {
            let cmd_enc = match device.create_command_encoder(&desc.label, queue_index) {
                Ok(cmd_enc) => cmd_enc,
                Err(e) => break 'error e,
            };
    ...
    pub fn device_create_render_bundle_encoder(
        ...
        *mut command::RenderBundleEncoder,
        ...
        let (encoder, error) = match command::RenderBundleEncoder::new(desc, device_id, queue_index)
        {
            Ok(encoder) => (encoder, None),
            Err(e) => (
                command::RenderBundleEncoder::dummy(device_id, queue_index),
                Some(e),
            ),
        };
    ...
    pub fn render_bundle_encoder_finish(
        ...
        let error = 'error: {
            let device = self.hub.devices.get(bundle_encoder.parent_id);
            ...
            let queue_index = bundle_encoder.queue_index;

            let render_bundle = match bundle_encoder.finish(desc, &device, queue_index, hub) {
                Ok(bundle) => bundle,
                Err(e) => break 'error e,
            };
