...
pub struct ShaderRuntimeChecks {
    ...
    /// If false, the caller **MUST** ensure that in all passed shaders every ray query
    /// has (wgsl naming) `rayQueryInitialize` must have called before `rayQueryProceed`, `rayQueryProceed`
    /// must have been called, returned true and have hit an AABB before `rayQueryGenerateIntersection`
    /// is called, `rayQueryProceed` must have been called, returned true and have hit
    /// a triangle before `rayQueryConfirmIntersection` is called, `rayQueryProceed`
    /// must have been called and have returned true before `rayQueryTerminate`,
    /// `getCandidateHitVertexPositions` or `rayQueryGetCandidateIntersection` is called,
    /// and `rayQueryProceed` must have been called and have returned false before `rayQueryGetCommittedIntersection`
